...
@Component
public class DailyTransactionAmountRule implements RuleValidator {

    private final ValidationRuleRepository ruleRepository;
    private final TransactionLogRepository logRepository;

    public DailyTransactionAmountRule(ValidationRuleRepository ruleRepository, TransactionLogRepository logRepository) {
    ...
    public ValidationResult validate(TransactionLogRequestDTO request) {
        ...
        BigDecimal totalToday = logRepository.sumAmountByUserIdAndTimeRange(request.getUserId(), start, end);

        BigDecimal newTotal = totalToday.add(request.getAmount());

        if (newTotal.compareTo(dailyLimit) > 0) {
            return ValidationResult.blocked(
                    "DAILY_TRANSACTION_AMOUNT_EXCEEDED",
